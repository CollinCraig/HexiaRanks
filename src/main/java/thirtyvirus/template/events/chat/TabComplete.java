...
public class TabComplete implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (command.getName().equals("hexpres")) {

            // no arguments
            if (args.length == 1){
                if (player.hasPermission("hexia.user")) { arguments.addAll(Arrays.asList("help", "info", "tutorial", "getpres", "master")); }
                if (player.hasPermission("hexia.admin")) { arguments.addAll(Arrays.asList("reload", "setpres", "reset")); }

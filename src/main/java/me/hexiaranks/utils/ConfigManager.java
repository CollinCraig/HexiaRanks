...
public class ConfigManager {
	...
	public ConfigManager(HexiaRanks main) {
		this.main = main;
	}
	...
	public File masterDataFile;
	public FileConfiguration masterDataConfig;
	public File masteryFile;
	public FileConfiguration masteryConfig;
	...
	public void loadConfigs() {
		...
		ignoredSections.add("Masterlist-gui.current-format.custom");
		ignoredSections.add("Masterlist-gui.completed-format.custom");
		ignoredSections.add("Masterlist-gui.other-format.custom");
		...
		if(!masterDataConfig.contains("players")) {
		masterDataConfig.options().copyDefaults(true);
		}
		if(!masteryConfig.contains("Mastery")) {
		masteryConfig.options().copyDefaults(true);
		}
	}
	public void saveConfigs() {
		...
		saveCustomYml(masterDataConfig, masterDataFile);
		saveCustomYml(masteryConfig, masteryFile);
	}
	...
	public void saveMasterDataConfig() {
		saveCustomYml(masterDataConfig, masterDataFile);
	}
	
	public void saveMasteryConfig() {
		saveCustomYml(masteryConfig, masteryFile);
	}
	...
	e.printStackTrace(); }
	 ...
	 private void createMasterDataConfig() {
	        masterDataFile = new File(main.getDataFolder(), "masterdata.yml");
	        if (!masterDataFile.exists()) { masterDataFile.getParentFile().mkdirs(); main.saveResource("masterdata.yml", false); }
	        masterDataConfig = new YamlConfiguration();
	            try {
					masterDataConfig.load(masterDataFile);
				} catch (FileNotFoundException e) { 	System.out.print("File Not Found <?>"); e.printStackTrace(); } catch (IOException e) {
	System.out.print("Read write Failed <!>"); e.printStackTrace(); } catch (InvalidConfigurationException e) { System.out.print("Corrupted Configuration File <?>");
	e.printStackTrace(); }
	}
	 private void createMasteryConfig() {
	        masteryFile = new File(main.getDataFolder(), "mastery.yml");
	        if (!masteryFile.exists()) { masterDataFile.getParentFile().mkdirs(); main.saveResource("mastery.yml", false); }
	        masteryConfig = new YamlConfiguration();
	            try {
    ...
    public void reloadConfigs() {
    	try {
			...
			messagesConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/messages.yml"));
			commandsConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/commands.yml"));
			//rankDataConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/rankdata.yml"));
			ranksConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/ranks.yml"));
			//prestigeDataConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/prestigedata.yml"));
			prestigesConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/prestiges.yml"));
			//masterDataConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/masterdata.yml"));
			masteryConfig = YamlConfiguration.loadConfiguration(new File("plugins/HexiaRanks/mastery.yml"));
    }

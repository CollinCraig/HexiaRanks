...
public class TopMasteryCommand extends BukkitCommand {

	private int masterPerPage;
	private String lastPageReached;
	private List<String> topMastery;
	...
	public TopMasteryCommand(String commandName) {
		super(commandName);
		this.setDescription(main.getStringWithoutPAPI(main.configManager.commandsConfig.getString("commands." + commandName + ".description", "top master leaderboard")));
		this.setUsage(main.getStringWithoutPAPI(main.configManager.commandsConfig.getString("commands." + commandName + ".usage", "/topmastery")));
		this.setPermission(main.configManager.commandsConfig.getString("commands." + commandName + ".permission", "HexiaRanks.topmastery"));
	...
	public String getName(int position) {
		return String.valueOf(main.lbm.getPlayerNameFromPositionMaster(position, "none"));
	}
	
	public String getValue(int position) {
		return String.valueOf(main.lbm.getPlayerMasterFromPosition(position, "none"));
	}
	
	public void load() {
		masterPerPage = 10;
		lastPageReached = main.prxAPI.g("top-mastery-last-page-reached");
		topMastery = main.prxAPI.h("top-mastery");
		leaderboardLines = new ArrayList<>();
		header = new ArrayList<>();
		footer = new ArrayList<>();
		topMastery.forEach(line -> {
        ...
        masterPerPage = leaderboardLines.size();
	}
  ...
  public void paginate(CommandSender sender, List<String> list, int page, List<String> header, List<String> footer, int size)
  {
      ...
      int cpos = (masterPerPage * (page - 1));
      if((size % masterPerPage) == 0)
      {
        if(size > 0)
        {
            totalPageCount = size / masterPerPage;
        }     
      }
      else
      {
        totalPageCount = (size / masterPerPage) + 1;
      }
 ...
 //endline
	...
	public boolean execute(CommandSender sender, String label, String[] args) {
		...
		if(args.length == 0) {
            this.paginate(sender, leaderboardLines, 1, header, footer, main.lbm.getMasterLeaderboard().size());
		} else if (args.length == 1) {
			this.paginate(sender, leaderboardLines, Integer.valueOf(args[0]), header, footer, main.lbm.getMasterLeaderboard().size());
		}
		return true;
	}

			...
			for(String rank : ranksCollection) {
				if(currentRankIndex == ranksCollection.indexOf(rank)) {
					...
					String format = main.prxAPI.cp(rankCurrentFormat.replace("%rank_name%", rank)
							...
							.replace("%nextrank_cost%", String.valueOf(main.prxAPI.getIncreasedRankupCostX(master ,prestige, main.rankStorage.getRankupCost(rankPath))))
							.replace("%nextrank_cost_formatted%", main.prxAPI.formatBalance(main.prxAPI.getIncreasedRankupCostX(master,prestige, main.rankStorage.getRankupCost(rankPath))))
                            , p);			

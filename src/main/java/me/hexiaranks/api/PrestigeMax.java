...
public class PrestigeMax implements IPrestigeMax {
	...
	public PrestigeMax(HexiaRanks plugin) {
		this.plugin = plugin;
	...
	public void execute(Player player) {
		...
		String masterName = getAPI().getPlayerMaster(uuid);
		String nextPrestigeDisplay = nextPrestige.getDisplayName();
		double nextPrestigeCost = getAPI().getIncreasedPrestigeCost(masterName, nextPrestigeName);
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
        	for(int i = currentPrestigeIndex ; i < prestigesCollection.size(); i++) {
        		...
        		double loopNextPrestigeCost = getAPI().getIncreasedPrestigeCost(masterName, loopNextPrestigeName);
       			...
       			if(!prestigeCommands.isEmpty()) {
       	           prestigeCommands.forEach(cmd -> {
       	        	   if(cmd.startsWith("[rankpermissions]")) {
       	        		   ...
       	        		   getAPI().allMasterAddPermissions.forEach(permission -> {
       	        			   plugin.perm.delPermission(p, permission);
       	        		   });
       	        	   } else {

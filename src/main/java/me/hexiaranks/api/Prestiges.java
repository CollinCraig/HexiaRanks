...
public class Prestiges {
	...
	private void sendList(CommandSender sender) {
		if(!enablePages) {
			...
			double firstPrestigeCost = main.prxAPI.getIncreasedPrestigeCost(master, main.prxAPI.getPrestigeCost(firstPrestige));
			...
			for(String prestige : prestigesCollection) {
				if(currentPrestigeIndex == prestigesCollection.indexOf(prestige)) {
					...
					String format = main.prxAPI.cp(prestigeCurrentFormat.replace("%prestige_name%", prestige2)
							...
							.replace("%nextprestige_cost%", String.valueOf(main.prxAPI.getIncreasedPrestigeCost(master, nextPrestige)))
							.replace("%nextprestige_cost_formatted%", main.prxAPI.formatBalance(main.prxAPI.getIncreasedPrestigeCost(master, main.prxAPI.getPrestigeCost(nextPrestige))))
                            , p);			
					...
					String format = main.prxAPI.cp(prestigeCompletedFormat.replace("%prestige_name%", prestige2)
							...
							.replace("%nextprestige_name%", nextPrestige)
							.replace("%nextprestige_displayname%", main.prestigeStorage.getNextPrestigeDisplayName(prestige2))
							.replace("%nextprestige_cost%", String.valueOf(main.prxAPI.getIncreasedPrestigeCost(master, main.prxAPI.getPrestigeCost(nextPrestige))))
							.replace("%nextprestige_cost_formatted%", main.prxAPI.formatBalance(main.prxAPI.getIncreasedPrestigeCost(master, main.prxAPI.getPrestigeCost(nextPrestige))))
                            , p);			
